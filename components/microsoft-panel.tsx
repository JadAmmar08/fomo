...
export function MicrosoftPanel({ roomId }: { roomId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/integrations/microsoft/folders?roomId=${roomId}`, { credentials: "include" })
      .then((r) => r.json())
      .then(setFolders)
  ...
  useEffect(() => {
    if (folders?.linkedFolderId) {
      fetch(`/api/integrations/microsoft/summary?roomId=${roomId}`, { credentials: "include" })
        .then((r) => r.json())
        .then(setSummary)
  ...
  async function linkFolder(folder: Folder) {
    setLinking(true);
    await fetch("/api/integrations/microsoft/folders", {
      method: "POST",
      credentials: "include",
  ...
  return (
    <section data-reveal style={{
      ...
      {!folders?.connected ? (
        <>
          <h2 style={{ marginBottom: 8 }}>Connect OneDrive.</h2>
          <p style={{ maxWidth: 480, marginBottom: 20 }}>
            Pick one shared folder this team already uses — FOMO only ever reads that folder. Read-only, never edits or deletes anything.
          </p>
          <a href={`/api/integrations/microsoft/connect?roomId=${roomId}`} className="button-secondary" style={{ display: "inline-flex" }}>
          ...
          <div style={{ display: "grid", gap: 8, maxHeight: 280, overflowY: "auto" }}>
            ...
            {folders.folders?.length === 0 && (
              <p style={{ color: "var(--subtle)", fontSize: "0.9rem" }}>
                No folders found in this OneDrive account.
              </p>
            )}
          </div>
        </>
